...
def train_SimpleRNN_Dense(x_train, y_train, x_test, y_test, scaler, seq_size):
    """Trains a model with two simple recurrent layers and one dense output layer
    ...
    """
    model = Sequential([
        keras.layers.SimpleRNN(20, input_shape=[None, seq_size], return_sequences=True),
    ...
    # model.load_weights('Weights/SimpleRNN_Dense_weights')
    model.summary()
    # history to visualise the loss, epochs the number of fits for the model
    history = model.fit(x_train, y_train, epochs=100, validation_data=(x_test, y_test))
    y_predict = model.predict(x_train)
    test_predict = model.predict(x_test)
    model.save_weights('Weights/SimpleRNN_Dense_weights')
